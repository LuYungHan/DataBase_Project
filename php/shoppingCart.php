...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($req == 'getcoupon') {
        ...
        //$result = $conn->query("SELECT * FROM coupon WHERE BuyerID='$userno' AND Used=0");
        $result = $conn->query("SELECT * FROM coupon WHERE Used=0");
        // =======================================================================================================
    ...
    if ($req == "purchaseshoppingcart") {
        ...
        if ($couponid == "") {
            ...
            $result = $conn->query("INSERT INTO Orders (BuyerID, `Date`, Price) VALUES ('$userno', '$date', '$total')");
            // ========================================================================================================================================
        } else {
            // ====================insert a new record into Orders=====================================================================================
            $result = $conn->query("INSERT INTO Orders(BuyerID, CouponID, `Date`, Price) VALUES ('$userno', '$couponid', '$date', '$total')");
            $conn->query("UPDATE Coupon SET Used=1 WHERE ID='$couponid'");
            // ========================================================================================================================================
        }
        // 找出 Orders 中最大的 ID ===========
        ...
        for ($i = 0; $i < count($row); $i++) {
            ...
            $rowitem = $conn->query("INSERT INTO order_list (OrderID, ProductID, `Amount`) VALUES ('$orderno', '$productID', '$quantity'");
            //有問題
            // =======================================================================================================
        }
